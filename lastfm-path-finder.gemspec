# Generated by jeweler
# DO NOT EDIT THIS FILE DIRECTLY
# Instead, edit Jeweler::Tasks in Rakefile, and run 'rake gemspec'
# -*- encoding: utf-8 -*-

Gem::Specification.new do |s|
  s.name = %q{lastfm-path-finder}
  s.version = "1.0.0"

  s.required_rubygems_version = Gem::Requirement.new(">= 0") if s.respond_to? :required_rubygems_version=
  s.authors = ["David J. Brenes"]
  s.date = %q{2012-07-28}
  s.description = %q{TODO: gem for finding paths between artists in Last.fm}
  s.email = %q{davidjbrenes@gmail.com}
  s.extra_rdoc_files = [
    "LICENSE.txt",
    "README"
  ]
  s.files = [
    "Gemfile",
    "README",
    "VERSION",
    "config/settings.example.yml",
    "lastfm-path-finder.rb",
    "lib/lastfm_path_finder.rb",
    "lib/lastfm_path_finder/artist.rb",
    "lib/lastfm_path_finder/finder.rb",
    "lib/lastfm_path_finder/path.rb",
    "lib/lastfm_path_finder/settings.rb",
    "spec/models/artist.rb",
    "spec/models/path.rb",
    "spec/spec_helper.rb"
  ]
  s.homepage = %q{http://github.com/brenes/lastfm-path-finder}
  s.licenses = ["MIT"]
  s.require_paths = ["lib"]
  s.rubygems_version = %q{1.6.2}
  s.summary = %q{TODO: Finding paths between artists in Lastfm since 1888}

  if s.respond_to? :specification_version then
    s.specification_version = 3

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0') then
      s.add_runtime_dependency(%q<lastfm>, [">= 0"])
      s.add_runtime_dependency(%q<rspec>, ["= 2.11"])
      s.add_runtime_dependency(%q<spork>, [">= 0"])
      s.add_runtime_dependency(%q<vcr>, [">= 0"])
      s.add_runtime_dependency(%q<webmock>, [">= 0"])
      s.add_runtime_dependency(%q<redis-objects>, [">= 0"])
      s.add_runtime_dependency(%q<rspec-redis_helper>, [">= 0"])
      s.add_runtime_dependency(%q<settingslogic>, [">= 0"])
      s.add_runtime_dependency(%q<commander>, [">= 0"])
      s.add_runtime_dependency(%q<jeweler>, [">= 0"])
      s.add_runtime_dependency(%q<simplecov>, [">= 0"])
    else
      s.add_dependency(%q<lastfm>, [">= 0"])
      s.add_dependency(%q<rspec>, ["= 2.11"])
      s.add_dependency(%q<spork>, [">= 0"])
      s.add_dependency(%q<vcr>, [">= 0"])
      s.add_dependency(%q<webmock>, [">= 0"])
      s.add_dependency(%q<redis-objects>, [">= 0"])
      s.add_dependency(%q<rspec-redis_helper>, [">= 0"])
      s.add_dependency(%q<settingslogic>, [">= 0"])
      s.add_dependency(%q<commander>, [">= 0"])
      s.add_dependency(%q<jeweler>, [">= 0"])
      s.add_dependency(%q<simplecov>, [">= 0"])
    end
  else
    s.add_dependency(%q<lastfm>, [">= 0"])
    s.add_dependency(%q<rspec>, ["= 2.11"])
    s.add_dependency(%q<spork>, [">= 0"])
    s.add_dependency(%q<vcr>, [">= 0"])
    s.add_dependency(%q<webmock>, [">= 0"])
    s.add_dependency(%q<redis-objects>, [">= 0"])
    s.add_dependency(%q<rspec-redis_helper>, [">= 0"])
    s.add_dependency(%q<settingslogic>, [">= 0"])
    s.add_dependency(%q<commander>, [">= 0"])
    s.add_dependency(%q<jeweler>, [">= 0"])
    s.add_dependency(%q<simplecov>, [">= 0"])
  end
end

